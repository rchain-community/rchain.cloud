...
class DockerRegistry {
  ...
  static getOrStartContainer (docker, image, tag, done) {
    const name = 'rchain-local-version-' + tag
    fs.readFile('/tmp/rchain/' + name + '.id', 'utf8', function (err, id) {
      ...
      docker.createContainer({
        ...
        if (!err) {
          fs.writeFile('/tmp/rchain/' + name + '.id', container.id, 'utf8', function(err, res){
