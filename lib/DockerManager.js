...
class DockerManager {
  ...
  static readFromLog(socket){
      fs.readFile('/tmp/rnode/rnode.log', 'utf8', function(err, data){
          if(err){
              console.log("Error reading rnode log file")
              throw err
          }
          let matches = data.match('RChain Node [0-99].[0-99].[0-99]')
          if(matches){
              console.log('Version before:' + rchainVersion)
              rchainVersion = matches[0].replace('RChain Node ', '')
              console.log('Version:' + rchainVersion)
              socket.emit('rchain.version', [rchainVersion])
